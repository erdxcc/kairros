...
import { DEFAULT_PALETTE, createPaletteCanvas } from './palette';
import {
    ADVECT,
    DISPLAY,
    DIVERGENCE,
...
import type { Pointer, ResolvedSilkOptions, SilkOptions } from './types';
...
export class SilkField {
    ...
    /**
     * `gl` and `renderer` are only null before mount() and after unmount(). Every
     * caller below runs between the two, so reaching these throws is a bug rather
     * than a state to handle.
     */
    private requireGl(): GL {
        if (!this.gl) throw new Error('SilkField: WebGL context used before mount()');
        return this.gl;
    }

    private requireRenderer(): Renderer {
        if (!this.renderer) throw new Error('SilkField: renderer used before mount()');
    ...
    private blit(mesh: Mesh, target: RenderTarget | null): void {
        this.requireRenderer().render({ scene: mesh, target: target ?? undefined });
    }
