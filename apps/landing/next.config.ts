 ...
 */
const REQUIRED_FOR_BUILD = ['NEXT_PUBLIC_SITE_URL', 'NEXT_PUBLIC_DASHBOARD_URL', 'NEXT_PUBLIC_NETWORK'];
...
export default function config(phase: string): NextConfig {
    if (phase === PHASE_PRODUCTION_BUILD) {
        ...
        if (missing.length > 0) {
            throw new Error(
                `${missing.join(' and ')} must be set for a production build of the landing site. NEXT_PUBLIC_SITE_URL is the canonical origin used for metadata, the sitemap and OG tags; NEXT_PUBLIC_DASHBOARD_URL is where Start, Sign in and Become a merchant point; NEXT_PUBLIC_NETWORK is the cluster named in the badges and in the integration snippet, and letting it default would mean labelling a mainnet site devnet. Set them in the deployment environment (see .env.example).`,
            );
        }
    }
    return nextConfig;
