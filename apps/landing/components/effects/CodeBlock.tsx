...
import { type TokenType, tokenize } from '@/lib/highlight';
...
// Tokens never reorder, but React still wants a stable key. The token's byte
// offset in the source is unique and survives re-renders.
function keyedTokens(code: string) {
    let offset = 0;
            ...
            {tabs.map((tab) => (
                ...
                >
                    <pre className="overflow-x-auto p-5 font-mono text-[13px] leading-relaxed">
                        <code>
                            {keyedTokens(tab.code).map((token) => (
